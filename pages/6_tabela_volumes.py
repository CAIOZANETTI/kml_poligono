...
st.markdown(
    ...
    "Todos os volumes sao **geometricos** (in-situ): a conversao de material "
    "(empolamento/contracao) depende do material real de cada trecho — solo, "
    "rocha alterada, rocha ou misturas — e pertence a uma analise de "
    "materiais separada."
)
st.markdown("---")
...
st.markdown(
    ...
    "plataforma e a geometria do lote. Tudo o que vem depois deriva destes "
    "valores."
)
st.markdown(
    "Poligono: **{nome}**  \n"
    ...
    "Remocao vegetal: **rv = {rv:.2f} m**".format(
...
# ── 5. Volume de corte ──
st.markdown("---")
st.markdown("#### 5. Volume geometrico de corte")
st.markdown(
    ...
    "esta acima do plano obtem-se o volume geometrico (in-situ). O talude de "
    "corte adiciona o material das bordas inclinadas que estabilizam a "
    "escavacao."
)
st.latex(r"V_{corte}^{plataforma} = \sum_{i \in corte} |\Delta h_i| \times A_{cel}")
v_corte_grade = r.volume_corte - r.volume_talude_corte
st.latex(r"V_{corte}^{plataforma} = %.2f \; m^3" % v_corte_grade)
if r.volume_talude_corte > 0:
    st.markdown("Com talude de corte:")
    st.latex(r"V_{corte} = %.2f + %.2f = %.2f \; m^3"
             % (v_corte_grade, r.volume_talude_corte, r.volume_corte))

st.latex(r"A_{corte} = %.2f \; m^2" % r.area_corte)

# ── 6. Volume de aterro ──
st.markdown("---")
st.markdown("#### 6. Volume geometrico de aterro")
st.markdown(
...
st.latex(r"V_{aterro}^{plataforma} = \sum_{i \in aterro} \Delta h_i \times A_{cel}")
v_aterro_grade = r.volume_aterro - r.volume_talude_aterro
st.latex(r"V_{aterro}^{plataforma} = %.2f \; m^3" % v_aterro_grade)
if r.volume_talude_aterro > 0:
    st.markdown("Com talude de aterro:")
    st.latex(r"V_{aterro} = %.2f + %.2f = %.2f \; m^3"
             % (v_aterro_grade, r.volume_talude_aterro, r.volume_aterro))

st.latex(r"A_{aterro} = %.2f \; m^2" % r.area_aterro)

# ── 7. Balanco de massa ──
st.markdown("---")
st.markdown("#### 7. Balanco de massa (geometrico)")
st.markdown(
    "O balanco confronta o que sai (corte) com o que entra (aterro) e responde "
    "a pergunta economica da obra: o material do proprio lote sobra, falta ou "
    "se equilibra? Subir a cota aumenta o aterro e reduz o corte; a **cota "
    "otima** e aquela em que o balanco se aproxima de zero — a terra do corte "
    "alimenta o aterro sem transporte externo. Importante: este balanco compara "
    "volumes geometricos; a equivalencia real corte↔aterro depende do material "
    "(empolamento/contracao) e deve ser tratada na analise de materiais."
)
st.latex(r"B = V_{corte} - V_{aterro} = %.2f - %.2f = %.2f \; m^3"
         % (r.volume_corte, r.volume_aterro, r.balanco_massa))
...
st.markdown("---")
st.caption(
    "Volumes geometricos (in-situ), sem fatores de conversao de material. "
